...
export default function EditPlotPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (rsNumber) {
        router.push(`/dashboard/land/plots/${params.id}`);
      } else {
        router.push('/dashboard/land/rs-numbers');
      }
    } catch (error: any) {
    ...
    <div className="max-w-2xl">
      <div className="mb-6">
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-2">
          <Link href="/dashboard/land/rs-numbers" className="hover:text-indigo-600">
            RS Numbers
          </Link>
          <span>/</span>
          <Link
            href={`/dashboard/land/rs-numbers/${rsNumber._id}`}
            className="hover:text-indigo-600"
          >
            {rsNumber.rsNumber}
          </Link>
          <span>/</span>
          <Link href={`/dashboard/land/plots/${params.id}`} className="hover:text-indigo-600">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        <div className="flex gap-3 mt-6">
          ...
          <Link
            href={`/dashboard/land/plots/${params.id}`}
